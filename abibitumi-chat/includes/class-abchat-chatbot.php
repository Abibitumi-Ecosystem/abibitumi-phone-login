...
class ABChat_Chatbot {
	...
	public function respond( $conversation_id, $text, $flow_id = '' ) {
		/**
		 * Filter the flow list the bot answers from.
		 *
		 * ABChat_Proactive hooks this to expose proactive quick replies as
		 * flows, so a button click is resolved by the same rule engine.
		 *
		 * @param array $flows Configured flows.
		 */
		$flows   = (array) apply_filters( 'abchat_bot_flows', (array) ABChat_Settings::get( 'bot_flows' ) );
		$matched = null;
